...
def read(json_data,config_file_path,task_id,run_id,paths_data,file_path,iter_value) -> bool:
    ...
    try:
        ...
        postgres_conn,_ = establish_conn_for_postgres(json_data, 'source',config_file_path)
        ...
        count1 = 0
        if source["query"] == "":
            task_logger.info("reading from postgres table: %s",
            source["table_name"])
            ...
            for query in pd.read_sql_table(source["table_name"], postgres_conn,\
            ...
            postgres_conn, chunksize = source["chunk_size"]):
                count1+=1
                task_logger.info('%s iteration' , str(count1))
                yield query
