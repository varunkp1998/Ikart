"""script to download and create folder structure"""
import sys
import logging
import os
import json
...
def create_folder_structure(prj_nm, path: str,paths_data:str):
    ...
    try:
        ...
        program_path = os.path.join(path,paths_data["programs"], prj_nm)
        ...
        folder_structure = [
            ...
            paths_data["logs"], paths_data["rejected"], paths_data["source_files"],
            paths_data["target_files"], paths_data["reports"]]),
            (os.path.join(task_path, paths_data["json"]), []),
            (os.path.join(task_path, paths_data["logs"]), []),
            (os.path.join(task_path, paths_data["text"]), []),
            (os.path.join(path, paths_data["src"], paths_data["connection"]), []),
...
def required_files_download(paths_data:str,repo_name,branch):
    ...
    try:
        ...
        if not Path(home_path+paths_data["engine_path"]+'engine_code.py').exists():
            try:
                main_logger.info("downloading of engine_code started...")
                download_file_from_github(repo_name,branch, file_path=paths_data[
                    "gh_engine_path"],save_dir =home_path+paths_data["engine_path"])
            except Exception as error:
                main_logger.info("error in downloading of engine_code %s", str(error))
        if not Path(home_path+ paths_data["dq_scripts_path"]+'checks_mapping.json').exists():
            main_logger.info("downloading of checks_mapping json started...")
            download_file_from_github(repo_name,branch, file_path=paths_data[
                "gh_checks_mapping_path"],save_dir =home_path+ paths_data["dq_scripts_path"])
            main_logger.info("downloading of checks_mapping json completed!")
        if not Path(home_path+paths_data["engine_path"]+'mapping'+JSON).exists():
            main_logger.info("downloading of mapping json started...")
            download_file_from_github(repo_name,branch, file_path=paths_data[
                "gh_mapping_path"],save_dir =home_path+paths_data["engine_path"])
            main_logger.info("downloading of mapping json completed!")
        if not Path(home_path+paths_data["dq_scripts_path"]+'definitions_qc.py').exists():
        ...
        if not Path(home_path+paths_data["orchestration_path"]+'orchestrate.py').exists():
            ...
            download_file_from_github(repo_name,branch, file_path=paths_data[
                "gh_orchestrate_path"],save_dir =home_path+paths_data["orchestration_path"])
...
def execute_pipeline_download(prj_nm,config_path:str,task_name:str,pipeline_name:str,run_id:str,
    ...
    try:
        ...
        orchestration_script=homepath+config_path["orchestration_path"]
...
def execute_engine(prj_nm,task_name:str,config_path:str,run_id:str,file_path,iter_value):
    ...
    try:
        logging_path= os.path.expanduser(config_path["folder_path"])+config_path["programs"]+prj_nm+config_path["task_log_path"]
        ...
        new_path = os.path.expanduser(config_path["folder_path"]) +config_path["engine_path"]
