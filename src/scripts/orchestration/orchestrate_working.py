...
def execute_job(prj_nm,paths_data,task_id,run_id,file_path,iter_value):
    ...
    try:
        logging_path= os.path.expanduser(paths_data["folder_path"])+paths_data["programs"]+prj_nm+\
...
def pipeline_json_read(path_data,proj_nm,pip_nm):
    ...
    try:
        new_path= os.path.expanduser(path_data["folder_path"])+path_data["programs"]+proj_nm+\
...
def orchestration_execution(prj_nm,paths_data,pip_nm,run_id,iter_value):
    ...
    file_path=os.path.expanduser(paths_data["folder_path"])+paths_data["programs"]+prj_nm+\
...
def restart_orchestration_execution(prj_nm,paths_data,pip_nm,run_id,iter_value):
    ...
    text_filepath=os.path.expanduser(paths_data["folder_path"])+paths_data["programs"]+prj_nm+\
...
def create_status_txt_file(paths_data1,task_name,proj_nm,run_id1):
    ...
    try:
        ...
        file_path1=os.path.expanduser(paths_data1["folder_path"])+paths_data1["programs"]+proj_nm+\
...
def pipeline_execution(proj_nm,path_data,pipe_nm,run_id1,log_file_path1,log_file_name1,
    ...
    try:
        ...
        df_1['Job_Status']= 'Start'
        file_path=os.path.expanduser(path_data["folder_path"])+path_data["programs"]+proj_nm+\
        ...
        text_filepath=os.path.expanduser(path_data["folder_path"])+path_data["programs"]+proj_nm+\
...
mode,iter_value):
    ...
    try:
        if task_nm != None:
            task_orc_execution(prj_nm,paths_data,task_nm,run_id,log_file_path,log_file_name,iter_value)
        elif pip_nm != None:
            pipeline_orc_execution(prj_nm,paths_data,pip_nm,run_id,log_file_path,log_file_name,mode,iter_value)
        else:
