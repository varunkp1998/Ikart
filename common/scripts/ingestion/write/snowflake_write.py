...
def insert_data(json_data,conn_details,datafram,conn,schema_name):
...
def trgt_record_count(json_data,status,conn2,task_id,run_id,iter_value,audit,conn_details):
...
def write(json_data, datafram,counter,config_file_path,task_id,run_id,paths_data,
    ...
    try:
        ...
        if json_data["task"]["target"]["operation"] == "create":
            ...
            # sys.exit()
            status = False
        trgt_record_count(json_data,status,conn2,task_id,run_id,iter_value,audit,conn_details)
        conn2.dispose()
        return status
