...
def establish_conn(json_data: dict, json_section: str,config_file_path:str):
    ...
    try:
        ...
        # log2.info("aws access key id %s", decrypt(connection_details["access_key"]))
...
def read(json_data: dict,config_file_path,task_id,run_id,paths_data,file_path,
    ...
    try:
        ...
        conn,connection_details = establish_conn(json_data,'source',config_file_path)
        bucket_name = connection_details["bucket_name"]
        # line of code for extension = '.'+json_data['task']['source']['file_type']
        ...
        if all_files == []:
            ...
            for file in all_files:
                ...
                for line in file_object.iter_lines():
                    line_count += 1
                # Subtract 1 if the file contains a header row
                line_count -= 1
                audit(json_data, task_id,run_id,'SRC_RECORD_COUNT',line_count,
                iter_value)
                row_count = line_count-default_skip_header-default_skip_footer
                count1 = 0
                if json_data["task"]["source"]["select_columns"] != " " and \
                json_data["task"]["source"]["alias_columns"] != " ":
                    default_header = 0
                    for chunk in pd.read_csv(conn.get_object(Bucket=bucket_name, Key=file)['Body'],
                    names = default_alias_cols,header = default_header,sep = default_delimiter,
                    usecols = default_alias_cols,skiprows = default_skip_header,nrows = row_count,
                ...
                json_data["task"]["source"]["alias_columns"] == " ":
                    ...
                    for chunk in pd.read_csv(conn.get_object(Bucket=bucket_name, Key=file)['Body'],
                    names = default_alias_cols,header = default_header,sep = default_delimiter,
                    usecols = default_select_cols,nrows = row_count,
