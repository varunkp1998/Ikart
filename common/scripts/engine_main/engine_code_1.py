...
def archive_files(inp_file_names, out_zip_file):
    ...
    log2.info("Archiving of file started %s-", inp_file_names)
    # create the zip file first parameter path/name, second mode
    #logging.info("Output Archive file is %s-", out_zip_file)
    zipf = zipfile.ZipFile(out_zip_file, mode="w")
    try:
        for file_to_write in inp_file_names:
            #logging.info(f' *** Processing file- {file_to_write}')
            zipf.write(file_to_write, file_to_write, compress_type=compression)
    except FileNotFoundError as error:
        log2.info("Exception occurred during Archiving process %s-", error)
    finally:
        zipf.close()
        #logging.info("Archiving file end time: %s", datetime.now().strftime("%d/%m/%Y %H:%M:%S"))
        log2.info("Archiving of file ended")
...
def engine_main(prj_nm,task_id,paths_data,run_id,pip_nm):
    ...
    try:
        if json_data['task']['src_archive_required'] == 'Y' and json_data['task']['source'][
            'source_type'] in ('csv_read','csvfile_read',"jsonfile_read","xmlfile_read","parquetfile_read","excelfile_read"):
            if json_data['task']['source']['source_type']=='csv_read' or json_data['task']['source']['source_type']=="csvfile_read":
                extension = ".csv"
            elif json_data['task']['source']['source_type']=="excelfile_read":
                extension = ".xlsx"
            input_path = json_data['task']['source']['source_file_path']+json_data[
            'task']['source']['source_file_name']+extension
            output_path = paths_data['folder_path']+paths_data['Program']+prj_nm+paths_data[
            'archive_path']+json_data['task']['source']['source_file_name']+".zip"
            log2.info(output_path)
            log2.info("archive_files calling")
            archive_files([input_path], output_path)
    except Exception as error:
        log2.exception("error in archiving file %s.", str(error))
        raise error
    ...
    try:
        ...
        try:
            if json_data['task']['tgt_archive_required'] == 'Y' and json_data['task']['target'][
            'target_type'] in ('csv_write','csvfile_write',"jsonfile_write","xmlfile_write",
            "parquetfile_write","excelfile_write"):
                input_path = json_data['task']['target']['target_file_path']+json_data[
                'task']['target']['target_file_name']
                output_path = paths_data['folder_path']+paths_data['Program']+prj_nm+paths_data[
                'archive_path']+json_data['task']['target']['target_file_name']+".zip"
                log2.info(output_path)
                archive_files([input_path], output_path)
        except Exception as error:
            log2.exception("error in archiving file %s.", str(error))
