...
def execute_query(prj_nm,paths_data:str,task_name:str,json_data,run_id,query_iteration,restart_point=None,task_iteration=None):
    try:
        print("query_ieration",query_iteration)
        print("restart point value is",(restart_point))
        print("type odf restart_point",type(restart_point))
    ...
    if restart_mode !="no restart":
        ...
        if audit_value_task != "COMPLETED":
            ...
            if response.status_code == 200:
                json_data = response.json()
                if isinstance(json_data, list):
                    audited_data = json_data  # Assuming the response is already a list of column values
